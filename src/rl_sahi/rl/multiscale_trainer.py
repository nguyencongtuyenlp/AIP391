...
from rl_sahi.rl.multiscale_env import MultiScaleYieldEnv, num_multiscale_actions, _MAX_SCALES
...
class MultiScaleEpisodeDataset:
    def __init__(self, image_root: Path, cache_root: Path, split: str, limit: int | None = None) -> None:
        ...
        if not self.items:
            raise FileNotFoundError(f"khong co multiscale cache cho split '{split}' tai {yroot}")
    ...
    def _load(self, dp: Path, yp: Path):
        ...
        z = np.load(yp)
        return det, z["cells"].tolist(), z["rois"], z["raw_yield"], z["real_yield"], z["small_gt_caught"], z["scales"]
...
def _make_env(epi, env_cfg, state_cfg) -> MultiScaleYieldEnv:
    det, cells, rois, raw, real, caught, scales = epi
    return MultiScaleYieldEnv(det, cells, rois, raw, real, caught, scales, env_cfg=env_cfg, state_cfg=state_cfg)
...
def _greedy_eval(policy, dataset, env_cfg, state_cfg, device, episodes):
    ...
    return {"val_recall": recall, "val_crops": avg_crops, "val_score": recall - float(env_cfg.crop_cost) * avg_crops / max(env_cfg.k_max, 1)}


def train_multiscale_dqn(
    ...
    dataset = MultiScaleEpisodeDataset(image_root, cache_root, split, limit=limit)
    val_dataset = None
    if getattr(cfg, "val_split", ""):
        try:
            val_dataset = MultiScaleEpisodeDataset(image_root, cache_root, cfg.val_split, limit=limit)
        except FileNotFoundError as exc:
            print(f"[multiscale] validation disabled: {exc}")

    num_actions = num_multiscale_actions(_MAX_SCALES)
    ...
    print(f"[multiscale] state_dim={state_dim} num_actions={num_actions} crop_cost={env_cfg.crop_cost} train_imgs={len(dataset)}")
    ...
    with log_path.open("w", newline="", encoding="utf-8") as f:
        ...
        for episode in range(1, cfg.episodes + 1):
            ...
            if episode % cfg.log_interval == 0 or episode == 1:
                print(f"[multiscale] ep={episode}/{cfg.episodes} reward={total_reward:.2f} loss={mean_loss:.4f} eps={row['epsilon']} crops={row['crops']} cov={row['covered']} valR={row['val_recall']} valC={row['val_crops']}")
